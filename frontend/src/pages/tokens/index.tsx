...
interface IssuedToken extends PersonalToken {
  secret: string
}
interface PersonalTokenService {
  key: string
  label: string
...
export function TokensPage() {
  ...
  const tokens = useQuery({
    ...
    queryFn: () => api.get<PersonalToken[]>('/api/v1/tokens'),
  })
  ...
  const create = useMutation({
    mutationFn: () =>
      api.post<IssuedToken>('/api/v1/tokens', {
        label: label.trim(),
        scopes,
    ...
    <div className="space-y-5">
      ...
      {tokens.isError && (
        <p role="alert" className="text-sm text-destructive">
          ...
          <Button variant="ghost" onClick={() => void tokens.refetch()}>
            Повторить
          </Button>
        </p>
      )}
      {tokens.data?.length === 0 && (
        <p className="py-8 text-sm text-text-muted">Токенов пока нет.</p>
      )}
      {tokens.data && tokens.data.length > 0 && filteredTokens.length === 0 && (
        <p className="py-8 text-sm text-text-muted">Токены по выбранным фильтрам не найдены.</p>
      )}
      ...
      {filteredTokens.length > PAGE_SIZE && (
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="text-center text-xs text-text-muted">
            {page * PAGE_SIZE + 1}–{Math.min((page + 1) * PAGE_SIZE, filteredTokens.length)} из{' '}
            {filteredTokens.length}
          </span>
          <Button
            variant="outline"
            className="h-10"
            disabled={(page + 1) * PAGE_SIZE >= filteredTokens.length}
        ...
        >
          ...
          <form className="space-y-4" onSubmit={submit}>
            ...
            <fieldset className="space-y-2">
              <legend className="text-sm font-medium">Доступы</legend>
              {services.data?.map((service) => (
                <div key={service.key} className="flex flex-wrap items-center gap-4 text-sm">
                  <span className="w-36">{service.label}</span>
                  {service.scopes.map((scope, index) => (
                    <label key={scope} className="flex min-h-10 items-center gap-2">
                      <input
                        type="checkbox"
                        disabled={create.isPending}
            ...
            {create.isError && (
              <p role="alert" className="text-sm text-destructive">
                Не удалось создать токен. Черновик сохранён; проверьте срок и выбранные доступы.
              </p>
            )}
            <DialogFooter>
              <Button type="submit" disabled={create.isPending || !scopes.length}>
      ...
      >
        ...
        >
          <DialogHeader>
            <DialogTitle>Секрет токена</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-text-muted">
            Секрет показывается только сейчас. Он не будет доступен после закрытия.
          </p>
          <code className="block break-all rounded border border-border bg-surface-raised p-3 text-xs select-all">
            {issued?.secret}
          </code>
          <DialogFooter>
            <Button
